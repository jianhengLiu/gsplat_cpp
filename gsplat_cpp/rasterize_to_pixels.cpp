...
//           means2d, conics, colors, opacities, backgrounds, masks, width,
...
//   auto backgrounds =
//   ctx->saved_data["backgrounds"].toOptional<torch::Tensor>(); auto masks =
//   ctx->saved_data["masks"].toOptional<torch::Tensor>();
...
//           means2d, conics, colors, opacities, backgrounds, masks, width,
//           height, tile_size, isect_offsets, flatten_ids, render_alphas,
//           last_ids, v_render_colors.contiguous(),
//           v_render_alphas.contiguous(), absgrad.requires_grad());
...
//     if (backgrounds.has_value()) {
//       auto backgrounds_sizes = backgrounds.value().sizes().slice(0,
//       -1).vec(); backgrounds_sizes.emplace_back(padded_channels); backgrounds
//       = torch::cat(
...
//       tile_size, isect_offsets.contiguous(), flatten_ids.contiguous(),
//       absgrad);
...
//     render_colors =
//         render_colors.slice(/*dim=*/-1, /*start=*/0,
//         /*end=*/-padded_channels);
//   }
